            ...
            $ionicPopup.alert({
                ...
                if($scope.user.sens){
                    Auth.ref().child(getAuth.uid).child('user-profil').set({ // ici on sélectionne l'id de l'user grace a Auth.getCurrentUserLoginData().uid,
                        ...
                        team: 'sens'// dans BDD, on a genre  =  firebase > UID > username & provider
                    });
                    MapService.markerArray().child('marker1').set({lat:48.89022 , lng: 2.34500});
                    MapService.markerArray().child('marker2').set({lat:48.88118 , lng: 2.31067});
